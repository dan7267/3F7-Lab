...
def huffman(p):
    ...
    while len(p) > 1:
        ...
        # we increase the variable nodelabel by 1 for its next use
        nodelabel += 1
        ...
        #...
        xt[len(xt) - 1][1] = [p[0][0], p[1][0]]

        # create a new entry pointing to the new node in the list of probabilities
        # This new e[1]ntry should be a tuple wi[1]th len(xt)-1 as its first element,
        # and the sum of the probabilities in p[0] and p[1] as[1] its second element
        #...
